  ...
  void activateUniforms(glm::vec4& _light0, glm::vec4& _light1, Material* m){
   ...
   void checkVisibility(glm::mat4 clipPlaneMatrix){
    ...
    for(int i = 0; i < teapotCount; i++)
    {
      ...
      glm::vec4 newTeapotPos = lookAtMatrix*glm::vec4(ut->position,1.0);
      glm::vec4 projectionVec = clipPlaneMatrix*newTeapotPos;  
      if(-projectionVec.w<projectionVec.x && projectionVec.x<projectionVec.w && -projectionVec.w<projectionVec.y && projectionVec.y<projectionVec.w && -projectionVec.w<projectionVec.z && projectionVec.z<projectionVec.w)
      {
